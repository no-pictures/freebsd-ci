...
#
# The BASIC-CI images boot with a serial console, DHCP, growfs and an
# sshd that accepts root with an empty password.
# The script boots the VM, waits for SSH, installs the SSH key and
# closes the empty-password access again.
# The forwarded SSH port only binds to 127.0.0.1, so the
# empty-password window is never reachable from outside the host.
set -e
. "$(dirname "$0")/config.sh"

if [ ! -f "${SSH_KEY}" ]; then
    ssh-keygen -q -t ed25519 -N "" -C "freebsd-ci" -f "${SSH_KEY}"
fi
